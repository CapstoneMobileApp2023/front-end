...
const ActivityEdit = ({currentUser, activities, updateActivity}) => {
    const { id } = useParams()

    let currentActivity = activities?.find((activity) => activity.id === +id)

    const [changeProfileAttribute, setChangeProfileAttribute] = useState({
      category: currentActivity?.name,
      activity: currentActivity?.username,
      start_time: currentActivity?.gender_identity
    })
